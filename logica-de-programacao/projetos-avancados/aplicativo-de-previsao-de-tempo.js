...
class Temperatura {
    constructor(dadosDoClima){
      this.dados = dadosDoClima
    }

    obterLocalizacao(){
      let cidade = this.dados.location.name
      let pais = this.dados.location.country

      return `${cidade}, ${pais}`
    }

    obterIcone(){
      let icone = this.dados.current.weather_icons
      return icone
    }

    obterTemperatura(){
      let temperatura = `${this.dados.current.temperature}&deg;C`
      return temperatura
    }
}
