...
class Passaporte {
    ...
    obterIniciais(){
        let iniciais = `${this.nome[0]}.${this.sobrenome[0]}.`
        return iniciais
    }

    obterEhNomeValido(){
        //pesquisar mais sobre endsWith() e startsWith()
        if (this.nome.length > 0 && this.sobrenome.length > 0 && !(this.sobrenome.endsWith("."))){
            return "Sim"
        }
        return "Não"
    }
