...
class ClimaDasCidades {
    ...
    exportarCSV(){
      let cidadesComTemperatura = this.cidades.map(function(cidade){
        return `${cidade.location.name} ${cidade.current.temperature}` //Cria uma string com o nome da cidade e sua temperatura na mesma linha
      })
      return cidadesComTemperatura.join(", ") // retorna a lista das string acima separada por ", ""
    }
}
